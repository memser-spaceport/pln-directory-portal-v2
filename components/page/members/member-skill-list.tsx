...
const MemberSkillList = (props: any) => {
  ...
  const remainingSkills = skills?.slice(noOfSkillsToShow, skills?.length).map((skill: any) => skill.title);
        ...
        {skills?.map((skill: any, index: number) => (
          <Fragment key={`${skill} + ${index}`}>
            {index < noOfSkillsToShow && (
              ...
              //     content={skill?.title}
              //   />
              <Tag title={skill?.title} value={skill?.title} variant={isListView ? '' : 'primary'} tagsLength={skills?.length} />
            )}
          </Fragment>
        ))}
        {skills?.length > noOfSkillsToShow && (
          
          <Tag variant="primary" value={'+' + (skills?.length - noOfSkillsToShow).toString()} title={remainingSkills.join(", ")}></Tag>
          // <Tooltip
          //   asChild
          //   trigger={
          //     <div>
