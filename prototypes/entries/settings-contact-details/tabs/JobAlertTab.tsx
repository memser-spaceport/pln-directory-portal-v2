'use client';

import { Button } from '@/components/common/Button';

// Production's own frame for this tab.
import s from '@/components/page/job-alerts/JobAlertsManager/JobAlertsManager.module.scss';

/**
 * COPY-SIMPLIFY of `JobAlertsManager`. Production reads `useJobAlerts` and
 * renders either a `JobAlertRow` or this empty state; the empty state is shown
 * here since the mocked member has no alert. Markup and copy verbatim.
 *
 * Included so the rail has no dead ends — every item a member can click leads
 * somewhere real.
 */
export function JobAlertTab() {
  return (
    <div className={s.root}>
      <h5 className={s.title}>Job Alert</h5>

      <div className={s.card}>
        <div className={s.content}>
          <div className={s.empty}>
            <div className={s.emptyText}>
              <p className={s.emptyTitle}>You don&apos;t have a job alert yet.</p>
              <p className={s.emptySub}>Browse jobs, apply some filters, and set a job alert to get a weekly email.</p>
            </div>
            <Button className={s.browseBtn} size="xs" variant="secondary" style="border">
              <span>Browse jobs</span>
              <ArrowUpRight />
            </Button>
          </div>
        </div>
      </div>
    </div>
  );
}

const ArrowUpRight = () => (
  <svg width="16" height="16" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path
      d="M14.344 4.5V11.8125C14.344 12.0363 14.2551 12.2509 14.0968 12.4091C13.9386 12.5674 13.724 12.6562 13.5002 12.6562C13.2764 12.6562 13.0618 12.5674 12.9036 12.4091C12.7454 12.2509 12.6565 12.0363 12.6565 11.8125V6.53906L5.09717 14.097C4.93866 14.2555 4.72368 14.3445 4.49951 14.3445C4.27535 14.3445 4.06036 14.2555 3.90185 14.097C3.74335 13.9384 3.6543 13.7235 3.6543 13.4993C3.6543 13.2751 3.74335 13.0601 3.90185 12.9016L11.4612 5.34375H6.18771C5.96394 5.34375 5.74933 5.25486 5.59109 5.09662C5.43286 4.93839 5.34396 4.72378 5.34396 4.5C5.34396 4.27622 5.43286 4.06161 5.59109 3.90338C5.74933 3.74514 5.96394 3.65625 6.18771 3.65625H13.5002C13.724 3.65625 13.9386 3.74514 14.0968 3.90338C14.2551 4.06161 14.344 4.27622 14.344 4.5Z"
      fill="currentColor"
    />
  </svg>
);
