...
import styles from './GenericRangeInput.module.scss';

export interface GenericRangeInputProps {
  /**
  ...
  /**
   * URL parameter key for minimum value
   */
  minParamName: string;

  /**
   * URL parameter key for maximum value
   */
  maxParamName: string;
  ...
  /**
   * Optional function to format display value
 ...
 *
 * Reusable component for min/max range inputs (e.g., price, size, age).
 ...
 *   allowedRange={{ min: 0, max: 1000000 }}
 ...
 *   minParamName="minCheckSize"
 *   maxParamName="maxCheckSize"
 *   filterStore={useFilterStore}
 *   allowedRange={{ min: 0, max: 10000000 }}
 * />
 * ```
 */
export function GenericRangeInput(props: GenericRangeInputProps) {
  const {
    label,
    minParamName,
    maxParamName,
  ...
  const getDisplayValue = (value: string, isFocused: boolean): string => {
    ...
    // Use custom formatter if provided, otherwise use currency
    if (formatValue && value) {
      const numericValue = parseFloat(value);
      return isNaN(numericValue) ? value : formatValue(numericValue);
    }
