...
const Toolbar = (props: IToolbar) => {
  //props
  const { handleAttendeesClick } = props;
            ...
            {isShowAttendeeListPopup && (
              <div className="toolbar__dropdown__attendee__type">
                ...
                {ATTENDEE_TYPE_DATA.map((item: any) => (
                  <div className="toolbar__dropdown__attendee__type__body__item" key={item.from}>
                    <input type="radio" name="attendees" value={item.from} checked={type === item.from || ((type === null || type === undefined) && item.from === 'upcoming')} onChange={() => onHandleAttendeesClick(item.from)} />
                    <label className="toolbar__dropdown__attendee__type__body__item__label" onClick={() => onHandleAttendeesClick(item.from)}>
                      {item.label}
                    </label>
                  </div>
                ))}
