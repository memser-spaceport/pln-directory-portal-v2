...
export function AdvisorCard({ advisor }: AdvisorCardProps) {
  ...
  const nextAvailable = getNextAvailableDate(advisor);
    ...
    <Link href={`/advisors/${advisor.id}`} className={styles.root}>
      ...
      <div className={styles.detailsSection}>
        <div className={styles.content}>
          <h3 className={styles.primaryText}>{advisor.member.name}</h3>
          <p className={styles.secondaryText}>
            {advisor.member.mainTeam?.name || advisor.member.teams?.[0]?.name}
          </p>
          <p className={styles.secondaryText}>
            {advisor.member.mainTeam?.role || advisor.member.role || ''}
          </p>
          {location && (
            <div className={styles.locationWrapper}>
              <LocationIcon />
              <p className={styles.tertiaryText}>{location}</p>
            </div>
          )}
        </div>
        <div className={styles.availabilityInfo}>
          ...
          <span className={styles.availabilityText}>
            {nextAvailable ? `First availability: ${nextAvailable}` : 'Request a time'}
          </span>
        </div>
      </div>
    </Link>
