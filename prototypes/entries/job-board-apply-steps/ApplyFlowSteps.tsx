 ...
 *
 * Same relationship `PendingApprovalSteps` has to `AppliedInvestorSteps`: the
 * markup is the source's, so the two stay diffable, and the day this app adopts
 * the token layer the fallbacks fall away and the DS component is what renders.
 *
 ...
 * `InvestorStepper`, `PendingApprovalSteps` — and it answers a different
 ...
 *
 * **`upcoming` covers blocked, too.** A pending-approval member cannot reach the
 * Application step, and the obvious move is a fourth status that paints it
 * refused. It isn't drawn, for the same reason production's stepper doesn't: a
 * step that is merely *not yet* and a step that is *waiting on the PL team* look
