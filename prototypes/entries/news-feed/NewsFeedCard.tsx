...
/**
 * Copy of the production NewsCard. The production card ends its meta line with
 * `StartConversationButton` (store / forum-access / analytics bound), swapped
 * here for a plain "Discuss" link. Adds a small follow button by the team name.
 */
...
export function NewsFeedCard({ item, following, onToggleFollow }: Props) {
    ...
    <div role="link" tabIndex={0} className={s.card} onClick={handleClick} onKeyDown={handleKeyDown}>
      <div className={s.head}>
        ...
        <a
          ...
          className={`${s.teamName} ${local.teamNameTight}`}
          onClick={(e) => e.stopPropagation()}
        >
          {item.teamName}
        </a>
        {/* Small follow button next to the team name. */}
        <span className={local.headFollow} onClick={(e) => e.stopPropagation()}>
          <FollowButton following={following} onClick={onToggleFollow} name={item.teamName} size="xs" tertiary bell />
        </span>
      </div>
      ...
      <div className={s.metaLine}>
        ...
        <button type="button" className={local.discuss} onClick={(e) => e.stopPropagation()}>
          Discuss
          <ArrowRight />
        </button>
      </div>
