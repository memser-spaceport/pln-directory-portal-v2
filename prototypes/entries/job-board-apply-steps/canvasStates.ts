...
export interface CanvasStateSpec {
   ...
   * its header; that door is the apply flow's `flow: 'details'` details step
   ...
   */
  flow?: 'review' | 'cv' | 'details' | 'experience' | 'application';
...
export const CANVAS_STATES: Record<string, CanvasStateSpec> = {
  ...
  'flow-details-empty': { viewer: 'logged-out', flow: 'details' },
  ...
  /* The middle step, reached on the way to an application rather than from a
     ...
     does not. */
  'drawer-pending-apply': { viewer: 'profile-incomplete', flow: 'details' },
  ...
  'import-review': { viewer: 'profile-incomplete', flow: 'details', import: { scenario: 'three-roles' } },
