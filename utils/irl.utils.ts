  ...
  return result.map((guest: any) => {
    ...
    return {
      ...
      events: validEvents.map((event: any) => ({
        ...
        hostSubEvents: event?.isHost ? [{ name: event?.name || '', link: '' }] : [],
        speakerSubEvents: event?.isSpeaker ? [{ name: event?.name || '', link: '' }] : [],
        sponsorSubEvents: event?.isSponsor ? [{ name: event?.name || '', link: '' }] : [],
