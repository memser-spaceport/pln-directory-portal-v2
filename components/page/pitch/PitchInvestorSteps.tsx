...
export const PitchInvestorSteps: React.FC<Props> = ({
  pitchSlug,
  prefillEmail,
  pitchStatus,
  ...
  const canAccessPitch = isLoggedIn && investorHasAccess && pitchStatus === 'OPEN';
  const isPitchAccessDenied = !!onContactSupport;

  const step1Status: StepStatus = isLoggedIn ? 'completed' : 'current';
  const step2Status: StepStatus = !isLoggedIn ? 'pending' : isProfileComplete ? 'completed' : 'current';
  const step3Status: StepStatus = !isLoggedIn ? 'pending' : canAccessPitch ? 'completed' : 'current';

  const step2Title = isProfileComplete ? 'Review your investor profile (optional)' : 'Set up investor profile';
  const step2Description = isProfileComplete
  ...
  const step2CtaLabel = isProfileComplete ? 'Go to Investor Profile' : 'Set Up Investor Profile';
  const step2ButtonClass = step2Status === 'completed' ? s.secondaryButton : s.primaryButton;

  const step3Title = isPitchAccessDenied ? "You don't have access to this pitch" : 'Access the team pitch';
  const step3Description = isPitchAccessDenied ? (
    <>
      Your account isn&apos;t on the invite list for this pitch page.
      <br />
      Contact support if you believe this is a mistake.
    </>
  ) : canAccessPitch ? (
    'You can view the pitch materials below.'
  ) : (
    'Once logged in, you can view the pitch materials below.'
  );
  const step3Height = isPitchAccessDenied ? 116 : 52;

  const steps = [
    {
      id: 1,
      status: step1Status,
      title: 'Log in with your whitelisted email',
      description: 'Use the email address that received the team pitch invite.',
      height: isLoggedIn ? 52 : 116,
      children: !isLoggedIn ? (
    ...
    },
    {
      ...
      height: isLoggedIn ? 116 : 52,
      children: isLoggedIn ? (
        <button type="button" className={step2ButtonClass} onClick={() => setDrawerOpen(true)}>
          <EditIcon /> {step2CtaLabel}
        </button>
      ) : undefined,
    },
    {
      ...
      height: step3Height,
      children: isPitchAccessDenied ? (
        <button type="button" className={s.secondaryButton} onClick={onContactSupport}>
          Contact support
        </button>
      ) : undefined,
    },
