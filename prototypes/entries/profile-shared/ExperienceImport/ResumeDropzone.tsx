...
// Chrome is production's own `FileUploader` stylesheet, imported rather than
// re-typed — the 16px row, its hover and `dragOver` borders, the 40px icon disc,
 ...
 * remove ✕.
 *
 * Production's uploader draws it with a local `CloseIcon` that is a *filled*
 * heavy cross (`FileUploader.tsx` L267), inheriting `--text-secondary`. Every
 * other ✕ in this product is the DS `CloseIcon`: a 1.5px stroked ✕ taking its
 * tone from `currentColor`. Two glyphs for one action is two controls as far as
 * the eye is concerned — the same swap, for the same reason, as
 * `SkillsTagsInput` one folder over.
 *
 * Also simplified, and said out loud rather than done quietly: **one file, no
 * video preview.** A résumé is singular, so `maxFiles`, the file *list*, and the
 * `showVideoPreview` branch are all answering a question this surface doesn't
 * have. Validation rules and their exact error strings are production's, word
 * for word, so a rejected file reads the same here as anywhere else.
 */
interface ResumeDropzoneProps {
  title: string;
  description: string;
  /** e.g. ['PDF', 'DOC', 'DOCX'] — extensions, upper case, as production takes them. */
...
export function ResumeDropzone(props: ResumeDropzoneProps) {
  ...
  /* Production's `validateFile`, verbatim — including the two message strings,
     which is the half that a reader actually meets. */
      ...
      >
        ...
        <input
          ...
          onChange={(ev) => {
            ...
            /* Production resets the value so the same file can be picked twice —
               which matters here, since "try again" is a real path. */
...
/** Production's own upload glyph, copied so the disc reads identically. */
