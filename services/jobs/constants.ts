 ...
 *
 * **Dark, and for a reason that is not "unfinished".** The layout is done; the
 * *content* does not exist. `IJobRole` carries no body at all — the board's rows
 * are scraped listing headers and the description has always lived behind the
 * outbound `applyUrl`. The one adjacent field anywhere is `summary` on the
 * backend's `JobOpening`, which arrives straight from the ingest payload, is
 * generated by nothing, and is not even selected by the board's query service.
 *
 * So flipping this on today would insert a reading step in front of Apply and
 * give most people nothing to read — a strictly worse funnel than the direct
 * Apply it replaces. It flips when the ingest carries a body, not before.
 *
