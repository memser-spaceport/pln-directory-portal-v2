...
function DirectoryResults({ actions }: RelatedResultsProps) {
  ...
  const noOfItemsToShow = 6;

  const onOpenDirectoryResultsPopup = () => {
    directoryResultsPopupRef.current?.showModal();
  };

  const onCloseDirectoryResultsPopup = () => {
    ...
    <>
      <div className="related-results">
        <div className="related-results__header">
          ...
          <button onClick={onOpenDirectoryResultsPopup} className="related-results__header__show-all">
            Show all ({actions.length})
          </button>
        </div>
        <div className="related-results__cn">
          {actions.slice(0, noOfItemsToShow).map((action: any, index: number) => (
            <a target="_blank" href={action.source} onClick={() => onActionCardClicked(action)} className="related-results__cn__item" key={index}>
              <div className="related-results__cn__item__wrpr">
                <div className="actions__cn__item__name__iconWrpr">
                  ...
                  {action.type?.toLowerCase() === 'event' ||
                    (action.type?.toLowerCase() === 'irl-event' && <img className="actions__cn__item__name__icon" src="/icons/irl-event-default-logo.svg" alt="icon" />)}
                </div>
                <div className="">
                  <p className="related-results__cn__item__name">
                    <span>{action.name}</span>
                  </p>
                  <p className="action-type">{action.type ? action.type === 'irl-event' ? 'Event' : action.type : ''}</p>
                </div>
              </div>
      ...
      <dialog onClose={onCloseDirectoryResultsPopup} ref={directoryResultsPopupRef} className="directoryResults-popup">
        <div className="directoryResults-popup__wrpr">
        <div className="directoryResults-popup__header">
          <h3 className="directoryResults-popup__header__title">Directory results</h3>
          <button onClick={onCloseDirectoryResultsPopup} className="directoryResults-popup__header__close">
            <img src="/icons/close.svg" alt="close" />
          </button>
        </div>
        <div className="related-results__cn--popup">
      ...
      <style jsx>
        {`
          ...
          @media (min-width: 1024px) {
            .related-results__cn, .related-results__cn--popup {
