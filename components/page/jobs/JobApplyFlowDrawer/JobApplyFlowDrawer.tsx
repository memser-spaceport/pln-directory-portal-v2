...
export function JobApplyFlowDrawer(props: JobApplyFlowDrawerProps) {
  ...
  /* One open section at a time on the profile step, and the way back to it in
     ...
     is: the cards are in the pane and the bar that reports on them is not. See
     `SectionEditLock`. */
  const sectionEditLock = useSectionEditLockRegistry();
  /* The sticky header covers the top of the scrolling column, so "how much of
     the card is on screen" has to discount it. */
          ...
          makes them read as one screen rather than three. */}
            <div className={d.footer}>
              <div className={d.footerInner}>
                {/* The open section's status, on the profile step — the way back
                to a card that was scrolled away, and its Save. It takes the
                hint's place, so Continue keeps its end of the bar. */}
