    ...
    return (
      <div className={s.screenBg}>
        <div className={s.profileLayout}>
          <button className={s.profileBackBtn} onClick={() => setSubmitted(false)}>← Back to profile</button>
          <div className={s.profileSection}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '20px 0', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 14, color: '#64748b', margin: 0, maxWidth: 380, lineHeight: 1.6 }}>
                {advisor.name} will receive your request via LabOS. They'll follow up by email to schedule a session.
              </p>
              <button className={s.btnPrimary} onClick={() => setSubmitted(false)}>Back to profile</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className={s.profileSection}>
          <div className={s.ohHeader}>
            <div>
              ...
              <p className={s.ohProviderNote} style={{ color: '#0891b2' }}>Calendar not connected — request a time directly</p>
            </div>
          </div>

          <div className={s.noSlotsBlock} style={{ marginBottom: 12 }}>
            <p className={s.noSlotsText}>
              {advisor.name} hasn't connected a scheduling tool yet. You can send a direct request and they'll follow up by email.
            </p>
          </div>

          <div className={s.requestForm}>
            <div className={s.requestAdvisorRow}>
              <img src={advisor.avatar} alt={advisor.name} style={{ width: 28, height: 28, borderRadius: '50%', objectFit: 'cover' }} />
            ...
            <label className={s.requestFormLabel}>
              What would you like to discuss? <span style={{ color: '#94a3b8', fontWeight: 400, fontSize: 12 }}>(optional)</span>
            </label>
            <textarea
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button className={s.btnPrimary} onClick={() => setSubmitted(true)}>Send request</button>
            </div>
          </div>
        </div>
