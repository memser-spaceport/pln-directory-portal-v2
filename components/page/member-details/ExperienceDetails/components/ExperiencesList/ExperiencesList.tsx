  ...
  return (
    <div className={s.root}>
      <div className={s.header}>
        <h2 className={s.title}>Experience {data?.length ? `(${data.length})` : ''}</h2>
      ...
      {!isLoading && !data?.length && (
        <div className={s.emptyData}>
          <span className={s.label}>{isEditable ? 'Add your work experience' : 'Not provided'}</span>
...
const ExpIcon = () => (
  <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" className={s.expIcon}>
    <path
