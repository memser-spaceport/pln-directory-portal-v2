...
jest.mock('next/dynamic', () => () => {
  const MockEditor = ({ value, onChange }: { value: string; onChange: (value: string) => void }) => (
    <textarea aria-label="Mock Bio Editor" value={value || ''} onChange={(e) => onChange(e.target.value)} />
  );
...
function TestForm({ fieldName, label }: { fieldName?: string; label?: string }) {
  const methods = useForm({ defaultValues: { bio: 'Default bio', teamBio: 'Team bio' } });

  return (
    <FormProvider {...methods}>
      <BioInput name={fieldName} label={label} />
