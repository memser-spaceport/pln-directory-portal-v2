...
jest.mock('@/components/page/home/husky-dialog', () => ({
  __esModule: true,
  default: () => null,
}));
...
jest.mock('@/components/page/home/featured/scroll-to-top', () => ({
  __esModule: true,
  default: () => null,
}));
...
describe('Home page — Quick Actions is hidden', () => {
  ...
  test('does not render the Quick Actions section for a logged-in member', async () => {
  ...
  test('still renders the rest of the home page content', async () => {
    getCookiesFromHeadersMock.mockResolvedValue({
      isLoggedIn: true,
      userInfo: { uid: 'uid-1', name: 'Member' },
      authToken: 'token',
  ...
  test('does not render Quick Actions for a guest, who still sees the welcome block', async () => {
