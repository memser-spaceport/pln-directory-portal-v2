...
export default function JobBoardPrototype() {
   ...
   *
   * **Who arrives here.** Applying from a role used to open this modal too; that
   * case is the apply flow's own details step now, which is why the state below
   * is a plain boolean and the modal no longer takes a role. What is left is
   * everyone whose next act needs an account but isn't applying: the header and
   * banner `Sign up` presses, and — since Refer came back for logged-out
   * visitors — a press on a row's `Refer`, which cannot open the referral modal
   * without a name to sign the note with. All three want the same generic form,
   * because none of them is standing on a particular job. */
        ...
        <div className={contentCss.list}>
          {visibleGroups.map((group) => (
            <JobTeamGroupCard
              ...
              /* Not "show the button" — the row always shows it. This is only
                 whether the press opens the referral modal; logged out it opens
                 the sign-up door instead, which is the same door the header and
                 the banner press. */
              canRefer={isLoggedIn}
