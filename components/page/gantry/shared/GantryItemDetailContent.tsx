...
interface Props {
  readonly uid: string;
  readonly variant: GantryItemDetailVariant;
  readonly onDismiss: () => void;
  readonly headerStart?: ReactNode;
}

export function GantryItemDetailContent({ uid, variant, onDismiss, headerStart }: Props) {
  ...
  const authorBackTo = variant === 'drawer' ? `/gantry/dashboard?itemId=${uid}` : `/gantry/${uid}`;
  ...
  if (isLoading) {
    return (
      <div className={clsx(s.mainContent, variant === 'drawer' && s.drawerMain)}>
        {headerStart}
      ...
      <div className={clsx(s.mainContent, variant === 'drawer' && s.drawerMain)}>
        ...
        <div className={s.notFound}>
          ...
          {variant === 'page' ? (
            ...
            <button type="button" className={s.notFoundLink} onClick={onDismiss}>
              Close
            </button>
          )}
        </div>
  ...
  const handleArchiveConfirm = async () => {
    try {
      ...
      await archiveMutation.mutateAsync(
        variant === 'drawer' ? 'Archived from detail drawer' : 'Archived from detail page',
      );
      setIsArchiveModalOpen(false);
      onDismiss();
    } catch {
      ...
      <div className={clsx(s.mainContent, variant === 'drawer' && s.drawerMain)}>
        ...
        <div className={s.header}>
          ...
          <div className={s.meta}>
            ...
            <GantryItemAuthor author={item.createdBy} backTo={authorBackTo} />
            <span className={s.metaDot} aria-hidden />
            <BoostButton
              count={item.pinCount}
