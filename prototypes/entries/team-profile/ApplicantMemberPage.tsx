...
// Production's page column and section stylesheets, imported verbatim.
...
import profileCss from '@/components/page/member-details/ProfileDetails/ProfileDetails.module.scss';
...
/* Production's contact order and handle names (`contact-details/ContactDetails`). */
const VISIBLE_HANDLES = ['email', 'linkedin', 'telegram', 'twitter', 'discord', 'github', 'bluesky'] as const;
const SOCIAL_TO_HANDLE_MAP = {
  email: 'email',
 ...
 * the profile card.
 *
 * Section order and visibility are production's (`app/members/[id]/page.tsx`),
 * so the founder reads the same page the directory shows:
 *
 * - **Profile** — `ProfileDetails` + `MemberDetailHeader`, transcribed; Bio
...
export function ApplicantMemberPage({ applicant, application }: Props) {
  ...
  const member = {
    ...
    projectContributions: p.contributions.map((c) => ({
      ...
      projectUid: c.uid,
      role: c.role,
      currentProject: c.end === null,
      startDate: `${c.start}-01T00:00:00.000Z`,
      endDate: c.end ? `${c.end}-01T00:00:00.000Z` : null,
      project: { name: c.project, isDeleted: false, logo: null },
    })),
  } as unknown as IMember;

  const mainTeam = p.teams.find((t) => t.mainTeam) ?? p.teams[0];
  const otherTeams = p.teams
    .filter((t) => t.id !== mainTeam?.id)
    .map((t) => t.name)
    .sort();
  ...
  return (
    <div className={clsx(page.memberDetail__container, s.column)}>
      {/* ---------- Profile (ProfileDetails + MemberDetailHeader) ---------- */}
      <div className={profileCss.root}>
                ...
                {mainTeam ? (
                  <>
                    <div className={h.teams}>
                      <p className={h.teamsName}>{mainTeam.name}</p>
                      {p.teams.length > 1 && (
                        <CustomTooltip
                          forceTooltip
                          trigger={
                            <button type="button" onClick={(e) => e.preventDefault()} className={h.moreTeamsButton}>
                              +{p.teams.length - 1}
                            </button>
                          }
                          content={
                            <>
                              {otherTeams.map((name, i) => (
                                <div key={name}>
                                  {name}
                                  {i === otherTeams.length - 1 ? '' : ','}
                                </div>
                              ))}
                            </>
                          }
        ...
        {p.bio && (
          <div className={profileCss.bioContainer}>
            <div className={profileCss.bioTitle}>Bio</div>
            <ExpandableDescription>
              <div className={profileCss.bioContent} dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(p.bio) }} />
            </ExpandableDescription>
          </div>
        )}
      </div>
            ...
            <div className={office.content}>
              <div className={office.officeHoursSection}>
                  ...
                  <div className={office.description}>
                    ...
                    <div className={office.keywordsWrapper}>
                      ...
                      <span className={office.badgesWrapper}>
                        {p.officeHours.interest.map((item) => (
                    ...
                    <div className={office.keywordsWrapper}>
                      ...
                      <span className={office.badgesWrapper}>
                        {p.officeHours.helpWith.map((item) => (
                          <div key={item} className={office.badge}>
                            {item}
                          </div>
                        ))}
                ...
                <div className={office.primaryButtonWrapper}>
                  ...
                  <span className={office.subtext}>{p.officeHours.pastBookings} past bookings</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* ---------- Contact Details (contact-details view, approved viewer) ---------- */}
      <DetailsSection>
              ...
              <div className={contact.top}>
                <div className={contact.content}>
                  {VISIBLE_HANDLES.map((item) => {
                    const handle = handles[SOCIAL_TO_HANDLE_MAP[item]] ?? '';
                    return (
                      <Fragment key={item}>
                        <ProfileSocialLink
      ...
      <DetailsSection>
        <ContributionsList member={member} userInfo={VIEWER} isEditable={false} onAdd={() => {}} onEdit={() => {}} />
      </DetailsSection>

      {/* ---------- Repositories (RepositoriesList, visitor state) ---------- */}
      <DetailsSection>
        ...
        <div className={repo.root}>
          {p.repositories.length ? (
            <ul className={repo.list}>
              {p.repositories.slice(0, 5).map((item) => {
                const url = `https://github.com/${p.githubHandle}/${item.name}`;
                return (
                  <li key={item.name} className={repo.expItem}>
                    <RepoIcon />
            ...
            <div className={repo.emptyData}>
              <span className={repo.label}>
                {p.githubHandle ? 'No repositories to display, add new ones to your GitHub profile.' : 'Not provided'}
              </span>
            </div>
          )}
        </div>
...
/* ---------- Inline icons, transcribed from the production files ---------- */

/* MemberDetailHeader's pin (20px). */
const LocationIcon = () => (
