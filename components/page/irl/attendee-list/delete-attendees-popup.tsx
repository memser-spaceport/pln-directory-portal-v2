...
import { getFormattedDateString, getGatherings } from '@/utils/irl.utils';
...
const DeleteAttendeesPopup = (props: IDeleteAttendeesPopup) => {
  const eventDetails = getGatherings(props?.searchParams, props?.eventDetails?.events, props?.from);
          ...
          <div className="popup__body__members">
            {selectedGuests?.map((guest: IGuest, index: number) => {
              const events = guest?.events?.filter((event: IIrlEvent) =>
                eventDetails?.some((g: IIrlEvent) => g?.uid === event?.uid),
              );
              return (
