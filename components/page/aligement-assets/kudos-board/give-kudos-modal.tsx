...
export function GiveCommunityKudosModal({ open, onClose, recipients, poolRemaining }: IGiveKudosModalProps) {
  ...
  async function onSubmit(values: CommunityKudosFormValues) {
    try {
      ...
      toast.success(`Sent ${values.points} community points to @${selectedRecipient?.name ?? 'recipient'}. 🪙`);
      reset();
      onClose();
    } catch (err) {
  ...
  return (
    ...
    >
      <form className="modal" onSubmit={handleSubmit(onSubmit)} noValidate>
        <h2 id="give-kudos-title" className="modal__title">
        ...
        <div className="form-group">
          ...
          {selectedRecipient && (
            <div className="mention-preview">
              ...
              <span>@{selectedRecipient.name} will see your kudos and points on the shared board.</span>
            </div>
          )}
        </div>
        ...
        <div className="form-group">
              ...
              >
                {giftOpts.length === 0 ? (
                  <option value="" disabled>
                    No points left in your pool this round
                  </option>
                ) : (
                  <>
          ...
          <div className="pool-summary">
            <span>
              Pool before this gift: <strong>{poolRemaining} pts</strong>
            </span>
            <span>
              After this gift: <strong>{Math.max(0, poolRemaining - selectedPoints)} pts</strong>
            </span>
          </div>
        </div>
        ...
        <div className="modal__footer">
          ...
          <button type="submit" className="btn-primary" disabled={isSubmitting || !isValid || giftOpts.length === 0}>
      ...
      <style jsx>{`
        ...
        .modal {
          ...
          padding: 30px 32px;
          box-shadow:
            0 8px 40px rgba(27, 84, 255, 0.12),
