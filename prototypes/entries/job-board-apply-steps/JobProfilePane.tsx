...
/**
 * FORK ONLY. Which third of the profile this mount renders.
 *
 * The pane is unchanged apart from this: same cards, same editors, same draft,
 * same handlers. The five-step rail does not need three new components — it
 * needs the one component to show a third of itself at a time, which is a prop.
 * Building three panes would have made the same cards drift three ways.
 */
export type ProfileSection = 'cv' | 'details' | 'experience';

interface JobProfilePaneProps {
  /** FORK ONLY — see `ProfileSection`. */
...
export function JobProfilePane(props: JobProfilePaneProps) {
  const { section, draft, setDraft, editing, setEditing, pendingRoleTitle, pendingApproval = false, canvasImport } = props;
   ...
   */
  /* FORK ONLY. On the CV step the offer IS the step, so it is never conditional
     there — `profileIsBlank` decided *where* the importer lived when one pane
     held everything, and a dedicated screen has already made that decision.

     Worth noticing what that costs: the rule this replaced ("one offer, in the
     place it is most useful") existed so two doors could never appear at once,
     and a step whose only content is the door cannot honour it. Someone who
     already has a full work history still gets the screen. */
  const importAtTop = section === 'cv' ? true : profileIsBlank;
  ...
  return (
    <>
      ...
      <div className={fd.stepIntro}>
        ...
        {/* FORK ONLY: only the details step can carry it. On the CV and
            Experience steps the person is not being asked to "fill in your
            profile" — they are being shown one offer and one list — and a 20px
            instruction naming the whole record on a screen holding a third of it
            is the step promising more than it contains. */}
        {section === 'details' && profileIsBlank && (
          <h2 className={clsx(fd.stepTitle, d.stepTitleWithLede)}>Fill in your profile</h2>
        )}
        ...
        {pendingApproval ? (
          ...
          <p className={fd.lede}>
            {/* FORK ONLY. One lede per section, because three screens opening on
                the same sentence would say the same thing three times on the way
                to one letter — and each of these screens is about a different
                thing, which is the argument for splitting them at all. The
                details step keeps the original wording; the other two say what
                *their* screen is for. */}
            {section === 'cv'
              ? 'Bring your work history from a document, or skip and type it in.'
              : section === 'experience'
                ? 'Where you have worked, what you have built, and the code you keep.'
                : pendingRoleTitle
      ...
      {/* 0. Start with a document, when there is nothing to start from.
               **The argument for this position has lapsed.** The drawer's rule is
               that required things are asked for first, and an earlier pass used
               that rule to keep the importer *out* of this position — a third
               thing above the gates would bury them. That was answered by saying
               the importer fills the required `role`, so it was the shortest
               route through a requirement rather than a third thing above one.

               The role is not required any more: the status is the only gate, and
               nothing a document contains can answer it. In this fork the point
               is nearly moot — the CV lives on its own step — but the card is
               still a quiet white one rather than a tinted slab, because it is an
               offer and the requirement one step on is a requirement. */}
      {/* FORK ONLY: the CV card is the CV step, and only the CV step. */}
      {section === 'cv' && importAtTop && (
        <DetailsSection
      ...
      {/* FORK ONLY — never renders here, and that is the point worth recording.
          The rule said "or fill it in yourself", naming the form *below it* as
          the alternative to the document. On the CV step there is no form below
          it — the fields are a screen away — so the sentence would be pointing
          at nothing. With one path and nothing to contrast it against there is
          no "or" to name, which is the same reason it went when the LinkedIn
          door was removed. The `Skip` button in the footer is the alternative
      ...
      {/* FORK ONLY: the answer applying requires is on this step and nothing
          else that gates anything is. The header card and contact details ride
          with it — production's own order, and neither asks for anything — so
          the step is "who you are, and the one thing we need from you". (It was
          two until the current role stopped gating; the role is still asked for
          on the header card, unmarked.) A Fragment rather than a wrapper div so
          all three stay direct flex children of the drawer's column and keep its
          16px gap. */}
      {section === 'details' && (
      <>
      ...
      {/* FORK ONLY: cards 3, 4 and 5 are the experience step. None of them gates
          anything — see the footer's note on what that makes this screen. */}
      {section === 'experience' && (
      <>
