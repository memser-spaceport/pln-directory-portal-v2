      ...
      <div className="tag-loading-container">
        ...
        <div className="tag-loading-container__tags">
          {Array.from({ length: 3 })?.map((firstSet, index) => (
            <div key={`${firstSet} ${index}`} className="tag-loading-container__tags__tag">
              {' '}
            </div>
          ))}
          {Array.from({ length: 3 })?.map((secondSet, index) => (
            <div key={`${secondSet} + ${index}`} className="tag-loading-container__tags__tag1">
              {' '}
            </div>
          ))}
          {Array.from({ length: 3 })?.map((thirdSet, index) => (
            <div key={`${thirdSet} + ${index}`} className="tag-loading-container__tags__tag2">
              {' '}
            </div>
          ))}
        </div>
