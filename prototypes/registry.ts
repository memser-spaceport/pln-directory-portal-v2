...
export const prototypeRegistry: PrototypeEntry[] = [
  ...
  },
  {
    ...
    title: 'Job Board',
    description:
      'Faithful mocked copy of the production /jobs page, carrying the apply flow. Logged out, one standing banner makes the offer — "Sign in to apply for N open roles with one click", counting whatever the rail is currently showing, with both Sign up and Sign in. Every role row leads with a real Apply button, keeping the arrow beside it as the way out to the posting; pressing Apply signs you in, then opens a profile drawer. The drawer is the member profile itself, card for card (header, Job search status, Experience, Project Contributions, Repositories) on production\'s own DetailsSection chrome, and it opens on that stack. Two answers gate applying — your current role and a private "Job search status" (actively looking / open to the right role / not looking, marked PL Team only and shown nowhere on the profile); everything else is optional. Experience can be typed or brought: its empty state carries the door production drew a `.connectButton` slot for and never wired — "Upload your CV", which takes a PDF, DOC or DOCX (a LinkedIn "Save to PDF" export among them, since a handle can never be read into a work history). What comes back is shown as a review card — positions to tick or drop, a start date to supply when the document had none, skills as editable tags, and the role and location only when the profile is still missing them — before anything touches the profile. Once the section has entries the door goes and the offer survives as one line at the top of the Add Experience form. Save and the application you were on resumes: an apply modal reads your profile back with an Edit escape, takes a required cover letter, and the row flips to "Applied". Nothing on the board is hidden from a logged-out visitor — the gate is on applying, not browsing. The earlier matching answer to the same question ("Matches you" badges, a "Best match for me" sort, the nudge strip, the preferences modal) is gone, as is the timed dwell prompt: with an Apply button on every row the ask is already made at the moment of intent. Refer is unchanged and still signed-in only: the referral modal searches live directory members and prefills the hiring team\'s leads as recipients, alongside the share popover for LinkedIn / X / copy link.',
    category: 'Jobs',
