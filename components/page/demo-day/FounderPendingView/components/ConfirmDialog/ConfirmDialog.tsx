...
const WarningIcon = ({ color = '#1B4DFF' }: { color?: string }) => (
...
export const ConfirmDialog = (props: ConfirmDialogProps) => {
  ...
  return (
    ...
    >
      <div className={clsx(s.dialog, classes?.dialog)} onClick={(e) => e.stopPropagation()}>
        <div className={s.header}>
          <div className={clsx(s.iconContainer, classes?.iconContainer)}>
            <WarningIcon color={iconColor} />
          </div>
          <h3 className={clsx(s.title, classes?.title)}>{title}</h3>
        </div>

        <div className={s.content}>
          <p className={clsx(s.message, classes?.message)}>{message}</p>
        </div>

        <div className={clsx(s.actions, classes?.actions)}>
          <button
            type="button"
            className={clsx(s.cancelButton, classes?.cancelButton)}
            onClick={onCancel}
            disabled={isLoading}
          >
            {cancelText}
          ...
          <button
            type="button"
            className={clsx(s.confirmButton, classes?.confirmButton)}
            onClick={onConfirm}
            disabled={isLoading}
          >
