...
export const useQueryParams = () => {
  ...
  const setParam = useCallback(
    (key: string, value: string | null | undefined, options: { scroll?: boolean; replace?: boolean } = {}) => {
  ...
  const setParams = useCallback(
    (updates: Record<string, string | null | undefined>, options: { scroll?: boolean; replace?: boolean } = {}) => {
      const { scroll = false, replace = false } = options;
      const params = new URLSearchParams(searchParams.toString());
  ...
  const removeParam = useCallback(
    (key: string, options: { scroll?: boolean; replace?: boolean } = {}) => {
  ...
  const removeParams = useCallback(
    (keys: string[], options: { scroll?: boolean; replace?: boolean } = {}) => {
