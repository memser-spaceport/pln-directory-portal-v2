...
// Production's menu chrome (popup, items), imported so the two cannot drift.
...
/**
 * Copy of the production account menu
 * (components/core/navbar/components/AccountMenu/AccountMenu.tsx) behind the
 * header avatar, with the proposed copy:
 *
 *  - the first item reads **My profile** instead of the member's name — the
 *    avatar you just pressed already says whose menu this is, so the name told
 *    you nothing and hid what the item does;
 *  - **Settings**, not "Account Settings" — the page behind it holds more than
 *    the account (notifications, job alerts);
 *  - **Sign out**, not "Logout" — the house auth verbs (auth-copy-audit #1);
 *  - the dividers lose their "Support" / "Settings" labels: one item per group
 *    doesn't need a heading, and "Settings" over an item called Settings
 *    repeated itself.
 *
 * Deliberately simplified: no auth store, analytics or real sign-out. The
 * links go to the prototype versions of those pages where one exists.
 */
export function PrototypeAccountMenu({ initials }: { initials: string }) {
  return (
    <Menu.Root modal={false}>
      <Menu.Trigger className={local.navAvatar} aria-label="Account menu">
        {initials}
      </Menu.Trigger>
      <Menu.Portal>
        <Menu.Positioner className={menu.Positioner} align="end" sideOffset={10}>
          <Menu.Popup className={menu.Popup}>
            <Menu.Item className={menu.Item} render={<Link href="/prototypes/member-profile" />}>
              <UserIcon /> My profile
            </Menu.Item>
            <Menu.Separator className={local.accountSeparator} />
            <Menu.Item className={menu.Item} render={<Link href="/changelog" />}>
              <ChangeLogIcon /> Changelog
            </Menu.Item>
            <Menu.Separator className={local.accountSeparator} />
            <Menu.Item className={menu.Item} render={<Link href="/prototypes/settings-contact-details" />}>
...
/* Icons: production's, verbatim (they are local to AccountMenu.tsx, not exported). */
