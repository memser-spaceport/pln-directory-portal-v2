...
export const prototypeRegistry: PrototypeEntry[] = [
  ...
  },
  {
    ...
    description:
      'The header search dialog with Husky as a row in the results and a state of the same dialog, instead of production\'s two-column overlay (keyword list left, AI chat with a wordless empty state right). Idle: recent searches plus the Husky page\'s "Try asking or searching for" block with prompts phrased as directory finds. Typing: production\'s category chips and result rows over a mocked corpus, ending in one fixed row — Ask Husky about "<term>" — which replaces both of production\'s AI doors (the zero-results card and the commented-out "Try AI Search" button). Pressing it streams an answer with sources, "Results from the directory" and follow-ups, with Back to results and Continue in Husky. Feedback is an inline thumbs pair; a thumbs-down asks why under the answer instead of opening the 1–5 rating dialog. ⌘K opens it. Everything mocked.',
    category: 'Cross-product',
